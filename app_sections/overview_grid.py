...
def app():
    ...
    with year_col:
        year = st.selectbox('Year', ['Select'] + list(range(2013, 2018)))

    with month_col:
        if year == 2018:
            month = st.selectbox('Month', ['Select', 'January', 'February', 'March', 'April'])
        else:
            month = st.selectbox('Month', ['Select'] + months)

    if (year != "Select" ) and (month != "Select"):
